"""
South East London: Core20Plus5 population map with assets

What it does
- Downloads borough boundaries (LAD Dec 2023) and filters to a selected ICB
- Downloads LSOA Dec 2021 boundaries, clipped to the ICB envelope + geometry
- Loads SEL Core20Plus5 population from CSV and joins by LSOA code
- Plots Core20Plus5 population as a choropleth
...
CORE20PLUS5_CSV = "sel_core_20.csv"
OUTPUT_PNG = "sel_core20plus5_population.png"
FIGSIZE = (12, 12)
DPI = 200
...
REQUEST_TIMEOUT = 60
USER_AGENT = "London-Core20Plus5-mapper/1.0 (+python requests)"
...
def load_latest_lsoa_population_total(xlsx_url: str) -> pd.DataFrame:
    ...
    if not lsoa_col or not total_col:
        raise ValueError(f"Could not find LSOA or Total columns in sheet '{latest_sheet}'.")

    out = df[[lsoa_col, total_col]].copy()
    out["population"] = pd.to_numeric(out[total_col], errors="coerce")
    out = out.dropna(subset=["population"])
    print(f"[info] Using sheet '{latest_sheet}' and column '{total_col}' for population.")
...
def main() -> None:
    ...
    print("[info] Loading Core20Plus5 CSV...")
    core = pd.read_csv(CORE20PLUS5_CSV, dtype=str)
    core.columns = [c.strip() for c in core.columns]
    core_lsoa_col = find_col(core, ["LSOA_code", "LSOA 2021 Code", "LSOA21CD"])
    value_col = find_col(core, ["Core20Plus5_unique_population", "Core20Plus5", "population"])
    if not core_lsoa_col or not value_col:
        raise ValueError("Could not find LSOA_code or Core20Plus5 population column in CSV.")

    core[value_col] = pd.to_numeric(core[value_col], errors="coerce")
    core = core.dropna(subset=[value_col])

    lsoa = lsoa.merge(
        core[[core_lsoa_col, value_col]],
        left_on=lsoa_code_col,
        right_on=core_lsoa_col,
        how="left",
        validate="m:1",
    )
    ...
    print("[info] Plotting Core20Plus5 population...")
    ...
    cax.set_ylabel("Core20Plus5 population", fontsize=7)
    ...
    ax.set_axis_off()
    scope_title = SCOPE_ICB if SCOPE_ICB else "Greater London"
    ax.set_title(f"{scope_title}: Core20Plus5 Population")
